...
void xivfixes::mimalloc_allocator(bool bApply) {
    ...
    static std::optional<hooks::direct_hook<decltype(_recalloc)>> realloc_hook;
    ...
    if (bApply) {
        ...
        const char* reallocPtr = utils::signature_finder()
            .look_in(utils::loaded_module(g_hGameInstance), ".text")
            .look_for_hex("48 89 5C 24 ?? 55 56 57 41 54 41 55 41 56 41 57 48 83 EC ?? 33 DB 48 8B F2 48 8B F9 8B EB 48 85 C9 75 ?? 4D 85 C0 0F 84 ?? ?? ?? ?? 49 8D 40 ?? 49 85 C0 0F 85 ?? ?? ?? ?? 8D 41 ?? 4C 3B C0 48 8D 4A ?? 49 0F 47 C0 48 8D 78 ?? 48 03 CF 48 3B D1 0F 87 ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 85 C0 0F 84 ?? ?? ?? ?? 48 8D 5F ?? 48 F7 D7 48 03 D8 48 23 DF 48 89 43 ?? E9 ?? ?? ?? ?? 48 85 F6 75 ?? 48 83 E7 ?? 48 8B 4F ?? E8 ?? ?? ?? ?? E9 ?? ?? ?? ?? 4D 85 C0 0F 84 ?? ?? ?? ?? 49 8D 40 ?? 49 85 C0 0F 85 ?? ?? ?? ?? 48 8B C7 48 83 E0 ?? 4C 8B 68 ?? B8 ?? ?? ?? ?? 4C 3B C0 49 8B CD 4D 8B F5 49 0F 47 C0 4C 8D 60 ?? 48 8B C7 49 2B C5 48 89 44 24 ?? E8 ?? ?? ?? ?? 48 8B C8 4C 8D 7E ?? 49 8B C5 48 2B C7 48 03 C8 48 3B CE 48 0F 47 CE 4D 03 FC 48 89 4C 24 ?? 49 3B F7 76 ?? E8 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? E9 ?? ?? ?? ?? 49 8D 44 24 ?? 49 03 C5 48 3B C7 72 ?? E8 ?? ?? ?? ?? 49 8B D7 49 8B CD ?? ?? 89 44 24 ?? E8 ?? ?? ?? ?? 48 8B F0 48 85 C0 75 ?? E8 ?? ?? ?? ?? 8B 4C 24 ?? ?? ?? 49 8B CF E8 ?? ?? ?? ?? 48 8B F0 48 85 C0 74 ?? BD ?? ?? ?? ?? EB ?? 4C 8B F0 49 8B C4 48 F7 D0 49 3B F5 75 ?? 48 85 C7 75 ?? 48 8B DF EB ?? 48 8B 54 24 ?? 49 8D 7C 24 ?? 4C 8B 44 24 ?? 48 03 FE 48 23 F8 49 03 D6 48 8B CF E8 ?? ?? ?? ?? 85 ED 74 ?? 49 8B CE E8 ?? ?? ?? ?? 48 89 77 ?? EB ?? E8 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8B C3 48 8B 5C 24 ?? 48 83 C4 ?? 41 5F 41 5E 41 5D 41 5C 5F 5E 5D C3 48 89 5C 24 ?? 48 89 6C 24 ?? 48 89 74 24 ?? 57 41 54 41 55 41 56 41 57 48 83 EC ?? 33 FF 4D 8B F1 48 8B EA 48 8B D9 48 85 D2 74 ?? 33 D2 48 8D 47 ?? 48 F7 F5 49 3B C0 73 ?? E8 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? E9 ?? ?? ?? ?? 49 0F AF E8 48 85 DB 0F 84 ?? ?? ?? ?? 4C 8B FB 49 83 E7 ?? 49 8B 4F ?? E8 ?? ?? ?? ?? 41 B8 ?? ?? ?? ?? 4D 3B F0 41 8B D0 49 0F 47 D6 49 8D 70 ?? 48 2B F2 48 03 F0 48 85 ED 75 ?? 49 8B 4F ?? E8 ?? ?? ?? ?? 48 8B DF E9 ?? ?? ?? ?? 4D 85 F6 0F 84 ?? ?? ?? ?? 49 8D 46 ?? 49 85 C6 0F 85 ?? ?? ?? ?? 49 8B 47 ?? 4D 3B F0 4C 8B E0 48 89 44 24 ?? 4D 0F 47 C6 48 8B C3 49 2B C4 49 8B CC 48 89 44 24 ?? 4D 8D 68 ?? E8 ?? ?? ?? ?? 48 8B C8 4C 8D 7D ?? 49 8B C4 48 2B C3 48 03 C8 48 3B CD 48 0F 47 CD 4D 03 FD 48 89 4C 24 ?? 49 3B EF 76 ?? E8 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? EB ?? 49 8B C4 48 83 C0 ?? 49 03 C5 48 3B C3 73 ?? 49 8B CF E8 ?? ?? ?? ?? 4C 8B F0 48 85 C0 0F 84 ?? ?? ?? ?? 41 BF ?? ?? ?? ?? EB ?? E8 ?? ?? ?? ?? 49 8B D7 49 8B CC ?? ?? 89 44 24 ?? E8 ?? ?? ?? ?? 4C 8B F0 48 85 C0 75 ?? E8 ?? ?? ?? ?? 8B 4C 24 ?? ?? ?? EB ?? 4C 8B E0 44 8B FF 49 8B C5 48 F7 D0 4C 3B 74 24 ?? 75 ?? 48 85 C3 0F 84 ?? ?? ?? ?? 48 8B 54 24 ?? 49 8D 5D ?? 4C 8B 44 24 ?? 49 03 DE 48 23 D8 49 03 D4 48 8B CB E8 ?? ?? ?? ?? 45 85 FF 74 ?? 49 8B CC E8 ?? ?? ?? ?? 4C 89 73 ?? E9 ?? ?? ?? ?? E8 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? E9 ?? ?? ?? ?? 48 8B F7 4D 85 F6 74 ?? 49 8D 41 ?? 49 85 C6 75 ?? 41 B8 ?? ?? ?? ?? 48 8D 4D ?? 4D 3B F0 4D 0F 47 C6 4D 8D 70 ?? 49 03 CE 48 3B E9 76 ?? E8 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? EB ?? E8 ?? ?? ?? ?? 48 85 C0 74 ?? 49 8D 5E ?? 49 F7 D6 48 03 D8 49 23 DE 48 89 43 ?? EB ?? E8 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8B DF 48 85 DB 74 ?? 48 3B F5 73 ?? 48 2B EE ?? ?? ?? ?? 4C 8B C5 33 D2 E8 ?? ?? ?? ?? 48 8B FB 4C 8D 5C 24 ?? 48 8B C7 49 8B 5B ?? 49 8B 6B ?? 49 8B 73 ?? 49 8B E3 41 5F 41 5E 41 5D 41 5C 5F C3 ?? ?? ?? ?? ?? ?? ?? ?? C3")
            .find_one()
            .resolve_jump_target();

        if (!reallocPtr) {
            logging::E("{} Failed to find signature for _recalloc", LogTag);
            return;
        }

        // E8 ?? ?? ?? ?? 48 8B F0 48 85 C0 74 ?? 48 3B FB
        ...
        logging::I("{} Hooking _recalloc", LogTag);
        realloc_hook.emplace("_recalloc", reinterpret_cast<decltype(_recalloc)*>(reallocPtr));
        realloc_hook->set_detour(mi_reallocarray);
    } else {
